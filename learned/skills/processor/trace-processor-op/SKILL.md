...
3. **Read the output.** It prints each hop (target → root), then the **root op** (`operation0` of the parentless row) and the **root→target op trace**. A `NO ROW FOUND` note means the chain reached a SMID with no row in the table (e.g. aged out of the retained partitions). A hop whose `_note` flags a **non-UUID `{group_id}-hex` parent** means the walk legitimately ends there: some ops (e.g. `import`) dispatch children with a parent that is not a `processor_msg_id` and has no row to walk to, so that hop is the deepest knowable op — see [[../../../wiki/processor/tracing-processor-op-lineage#non-uuid-parent-the-walk-terminates-early|non-UUID parent]] (not a failure; the trace no longer errors on it).
