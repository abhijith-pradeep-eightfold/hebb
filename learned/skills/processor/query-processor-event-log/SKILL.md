...
2. **Run the bundled reader** with one or more filters:
   ...
   - Filters (all optional, AND-combined; **at least one is required** so the scan stays bounded): `--msg-id`, `--parent-msg-id`, `--group-id`, `--operation`, `--queue` (matched on the **trimmed** `queue_name` — the stored value may carry a trailing space), `--event-type` (e.g. `message_dispatched`), `--since` / `--until` (absolute `t_create` bounds, `'YYYY-MM-DD[ HH:MM[:SS]]'`), `--since-hours N`, `--limit N` (default 200), `--region <region>` (see below). `--format json` emits machine-readable rows.
   - **`--region <region>`** — sets `EF_DEFAULT_REGION` for this invocation so the adapter factory routes to the correct warehouse. Supported: `us-west-2`, `eu-central-1`, `ca-central-1`, `ap-southeast-2`, `westus2` (Azure/Databricks), plus any other region with a `REDSHIFT_LOG`-routed warehouse. When unset, `EF_DEFAULT_REGION` from the environment is used.
