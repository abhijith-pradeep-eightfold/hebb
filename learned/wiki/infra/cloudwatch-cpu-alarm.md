...
- [[../oncall/airflow-dag-failure|Airflow DAG Failure]] — a custom airflow-namespace per-DAG `...failed.sum` `Sum >= 1` alarm (no dimensions); the same CloudWatch read pattern with the namespace/metric swapped in.
