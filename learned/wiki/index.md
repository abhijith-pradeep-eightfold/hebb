...
- [[oncall/airflow-dag-failure|Airflow DAG Failure]] — the per-DAG failure-counter ticket type: a `[<region>] Airflow DAG Failure-<dag>` / `airflow-airflow.<dag>.failed.sum` `Sum >= 1` / 900s / 1-of-1 alarm. The crux: the counter is keyed on the wrapped script's **process exit code** (`run_cron_script.sh`: non-zero → `...failed.sum`, zero → `...success.sum`), so many exit-0 aborts never page and the alarm fires on the first non-zero-exit run; on-demand DAGs (`schedule_interval=None`, `retries=0`) make it intermittent. Distinct from *Alarm Provisioning Failures*. For a deploy DAG, confirm the deploy via the `build_log` table and route via CODEOWNERS.
...
- [[infra/build-log-table|build_log table (global db)]] — the **deployment record** (one row per build/deploy: `namespace`/`tag`/`git_revision`/`status`/compressed `data_json`) on the **`global`** db; the source of truth for "was a deploy actually triggered" (deploys propagate to all regions; alerts land in `#build_alerts`). Read via `BuildLog().load(... db='global')`; `SELECT *` over a window times out on the compressed `data_json`, so select scalar columns + push `LIKE`/range filters into SQL.
- [[infra/azure-app-deployer-resource-groups|Azure App Service deployer resource-group asymmetry]] — `AzureAppDeployer` pins **site/slot** lookups to a hardcoded `eightfold-infra-resource-group-west-2` with no fallback, while **plan/autoscale** lookups iterate both candidate RGs (`POSSIBLE_PLAN_RESOURCE_GROUPS`, the documented historical-naming two-RG world). So a site provisioned in `...westus2` yields a spurious `ResourceNotFound` — a plausible (source-supported, not live-confirmed) cause of an Azure-deploy `ResourceNotFound`.
