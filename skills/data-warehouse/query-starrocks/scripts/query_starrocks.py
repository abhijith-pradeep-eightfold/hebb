...
def _strip_comments(s):
    ...
    Both checks in ensure_read_only — the leader-keyword match and the
    multi-statement `;` scan — run on this comment-stripped copy, so a `;` (or a
    leading verb) that appears *inside a comment* is never mistaken for query
    syntax. We keep the original text intact for the executor; this copy is
    validation-only, so its crude handling can't mangle a string literal in the
