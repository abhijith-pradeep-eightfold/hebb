...
The **only** hard write-boundary is `inputs/` — immutable witness history. Everything else, `core/` included, is yours to change when the work calls for it. The discipline that matters is not *where* you may write, but *fixing the root cause*: when a compiled artifact is wrong, repair the generator too (next section), so the fix sticks across recompiles.
