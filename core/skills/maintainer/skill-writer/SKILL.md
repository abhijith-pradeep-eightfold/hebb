...
## Boundaries
- A compile writes **only** under top-level `skills/` (and `agents/` if A1 forces an agent), so the diff stays traceable to the session-doc; `inputs/` is immutable. You may *reference* a core skill here. The engine itself can be fixed — but that's a separate maintainer task (`core/CLAUDE.md` → *Fixing issues at the source*), not part of compiling this doc into a learned skill.
