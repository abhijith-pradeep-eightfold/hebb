...
6. **Spot skill opportunities, including chains.** Find the repeatable steps — anything a future agent would redo. For each, capture: what it does, the **full inline scratch-script** the log recorded, its env/`$CODE_BASE` coupling, and a light A4 read (looks like no coverage, or maybe already covered — flag it; `skill-writer` does the deep search).
   - **Decompose a task-specific candidate into its reusable building block.** When a candidate skill is really "do task T" wrapped around a generic lower-level capability (e.g. "read table/log X", "resolve routing for model Y", "look up host Z"), flag **both** for `skill-writer`: the task-specific skill *and* the extractable building block (name the capability, the lines of the script that implement it, and a suggested `learned/hebb_utils/<domain>/` home). The aim is that the building block becomes its own small skill + shared util so *other* use-cases reuse it — so surface it even when this doc has only one consumer of it. A monolith that hides a reusable capability is itself a finding to report. Also report the **ordered sequence of skill invocations** in the session, and for **every adjacent chain** (A→B) classify whether **any runtime judgment occurred between the steps**: did the agent decide what B does based on A's result, or were B's inputs a pure function of A's outputs (a deterministic hand-off)? Treat "entered B at a fixed entry point because we came from A" as *no* judgment — it is a constant consequence of the chain, not a decision.
   - **No judgment between steps** → flag a **fuse-into-combined-skill** opportunity: carry the constituent skills, their full scripts, and the reusable stages to extract into `learned/hebb_utils/`. `skill-writer` builds **one combined skill + one bundled script** (Rule A2/A4). **This does not require the chain to recur** — a no-judgment hand-off is the signal on its *first* occurrence, even when the chain ran at low effort with no intervention.
